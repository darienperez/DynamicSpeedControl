...
make_trace(cs::ClusteredState, cls; k::Int=2)  = begin
    transform(cs.pca, cls[k]) |> DataFrame |> df ->
        scatter3d(df, x=:x1, y=:x2, z=:x3, mode="markers", marker=attr(size=2, opacity=0.5))
end
